...
## Built — variant alongside the AB amp ✅
`kicad/power_amp_classa.kicad_sch` (generated by `kicad/gen/gen_classa.py`) is a
**standalone, ERC-clean** Class-A version of the power-amp sheet: the identical
LM1875 stage **plus** `U_ccs` (LM317 as a constant-current sink, `R_set` = 2.5 Ω
2 W → `I = 1.25/2.5 = 0.5 A` from `PA_OUT` to GND). It's a **drop-in alternative**
to the main `power_amp` sheet — the main board stays Class AB; swap this in to go
Class A.

**Parts the Class-A variant ADDS** over the AB power amp:
| Ref | Part | Note |
|-----|------|------|
| `U_ccs` | **LM317T** (2nd) | wired as a current source; **needs a heatsink** (~7.5 W) |
| `R_set` | **2.5 Ω 2 W** | sets `I_A` = 1.25/R_set = 0.5 A (use 2.4 Ω E24 ≈ 0.52 A) |
| heatsink | **≤1.5 °C/W** on both the LM1875 and the LM317 | the ~16 W standing heat splits across the two devices (~8 W each) |

Regenerate: `python3 kicad/gen/gen_classa.py` (writes the `.kicad_sch` + a minimal
`.kicad_pro`). `kicad-cli sch erc` → **0 violations**.
